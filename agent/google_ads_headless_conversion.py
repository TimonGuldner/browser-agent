...
async def wait_for_cdp(timeout: int = 40) -> None:
    ...
    while time.time() < deadline:
        try:
            r = httpx.get(CDP_URL + '/json/version', timeout=2)
...
async def ads_page() -> tuple[Any, Any, Page]:
...
async def verify_authenticated_and_account() -> dict[str, Any]:
    pw, browser, page = await ads_page()
    try:
...
async def set_control_near_text(page: Page, text_patterns: list[str], desired: bool) -> dict[str, Any]:
    """Toggle the first checkbox/radio/aria control located in a small ancestor around matching visible text."""
...
async def deterministic_source_fix() -> dict[str, Any]:
    ...
    try:
        ...
        analytics = await set_control_near_text(
            page,
            [r'Google Analytics', r'Google-Analytics'],
            True,
        )

        # Calls may already be off; only fail if we can see call text but cannot resolve its control.
        lower = body.lower()
        ...
        return {'ok': continued, 'reason': None if continued else 'CONTINUE_BUTTON_NOT_FOUND', 'calls': calls, 'analytics': analytics}
    finally:
        try:
            await browser.close()
        except Exception:
...
async def main() -> None:
    ...
    try:
        ...
        phase1 = await run_agent(
            browser_session,
            llm,
            f'''Work only in Google Ads account {ACCOUNT_ID} / LOCENIX. Do not alter campaigns, ads, keywords, budgets, bidding, billing, users, or spend settings. The campaign LOCENIX | Search | High Intent | DE must remain paused.

Navigate to Goals > Conversions > Summary. Verify the active customer is {ACCOUNT_ID}. If a suitable imported GA4 conversion named purchase already exists, stop and report exactly ALREADY_EXISTS plus whether it is Primary. Otherwise start New conversion action > Import and navigate only as far as the conversion-source selection screen where Google Analytics and other source types such as Calls/Anrufe are shown. Do NOT select, deselect, or toggle any conversion source on that screen. Stop there and report SOURCE_SCREEN_READY.''',
            24,
        )
        ...
        if not source_fix.get('ok'):
            write_result({'status': 'blocked', 'reason': source_fix.get('reason', 'SOURCE_FIX_FAILED'), 'auth': auth, 'phase1': phase1, 'source_fix': source_fix})
            return

        phase2 = await run_agent(
            browser_session,
            llm,
            f'''Continue ONLY the Google Analytics 4 conversion import already open in Google Ads account {ACCOUNT_ID} / LOCENIX. The conversion source selection has already been corrected so that Calls/Anrufe must NOT be imported.
...
HARD SAFETY: do not enable/pause/edit any campaign, ad group, ad, keyword, asset, budget, bid strategy, targeting, billing, payment, user/permission, or spend setting. Campaign `LOCENIX | Search | High Intent | DE` must remain PAUSED and no budget may change. Finish with `CONVERSION_DONE` only if purchase is visibly present after saving; otherwise report the exact blocker.''',
        ...
        success = 'CONVERSION_DONE' in phase2.get('final_result', '') or phase2.get('successful') is True
        write_result({
