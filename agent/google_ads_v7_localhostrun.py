...
SESSION_FILE = Path('/tmp/google_ads_v7_session.json')
CREDS_FILE = Path('google_ads_v7_credentials.json')
...
def write_result(payload: dict) -> None:
    ...
    print('GOOGLE_ADS_V7_RESULT=' + json.dumps(payload, ensure_ascii=False), flush=True)
...
def extract_tunnel_url(text: str) -> str:
    ...
    for url in candidates:
        ...
        if host == 'localhost.run':
            continue
        if host.endswith('.localhost.run') or host.endswith('.lhr.life') or host.endswith('.lhr.rocks'):
            return url.rstrip('/')
    return ''
...
def start() -> None:
    ...
    passwd_file = '/tmp/locenix-google-v7.pass'
    subprocess.run(
    ...
    pids.append(websockify.pid)

    tunnel_log = Path('/tmp/localhostrun-google-v7.log')
    log_handle = tunnel_log.open('wb')
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            tunnel = extract_tunnel_url(text)
            if tunnel:
                break
        except Exception:
            pass
    ...
    if not tunnel:
        diagnostic = ''
        try:
            diagnostic = strip_ansi(tunnel_log.read_text(errors='ignore'))[-1200:]
        except Exception:
            pass
        stop_pids(pids)
        raise RuntimeError('localhost.run tunnel could not be created: ' + diagnostic.replace('\n', ' ')[:1000])

    # Never expose a URL until the public endpoint itself returns the noVNC client.
    reachable = False
    status = None
    for _ in range(15):
        try:
            r = httpx.get(tunnel + '/vnc.html', timeout=10, follow_redirects=True)
            status = r.status_code
            content = r.text[:5000].lower()
            if r.status_code < 500 and ('novnc' in content or 'vnc' in content):
                reachable = True
                break
        except Exception:
            pass
    ...
    if not reachable:
        stop_pids(pids)
        raise RuntimeError(f'localhost.run URL was created but noVNC preflight failed (status={status})')
    ...
    print('LOCALHOSTRUN_LOGIN_BROWSER_READY=true', flush=True)
...
async def run() -> None:
    ...
    try:
        # Avoid treating the initial Ads redirect as an authenticated state.
        ...
        # Persist auth first. Any later retry can then run without another manual login.
