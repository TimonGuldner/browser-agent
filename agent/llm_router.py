...
def configured_slots(task_type: str = TASK_BROWSER_REASONING) -> list[ProviderSlot]:
    ...
    by_provider = {
        "google": _discover("GOOGLE_API_KEY", "google", GOOGLE_DEFAULT_MODEL),
    ...
    for slot in slots:
        ...
        try:
            ...
            meta.update({
                ...
                "provider_attempts": attempts + [{"provider": slot.provider, "key_slot": slot.key_slot, "status": "completed"}],
            ...
            err = str(exc)
            attempts.append({
                "provider": slot.provider, "key_slot": slot.key_slot, "status": "failed",
