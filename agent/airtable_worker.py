...
from agent.cost_gate import evaluate_inbox_gate, mark_inbox_llm_completed
...
# These are hard emergency ceilings, not targets. Normal Luna runs should be far below them.
ROLE_POLICIES: dict[str, dict[str, Any]] = {
    "inbox": {"max_steps": 24, "max_history_items": 10, "run_budget_usd": 0.25},
    "growth": {"max_steps": 40, "max_history_items": 12, "run_budget_usd": 0.75},
    "lead": {"max_steps": 40, "max_history_items": 12, "run_budget_usd": 0.75},
    "content": {"max_steps": 40, "max_history_items": 12, "run_budget_usd": 0.75},
}
DEFAULT_POLICY = {"max_steps": 30, "max_history_items": 12, "run_budget_usd": 0.50}

# Official GPT-5.6 Luna token prices, used only for a local safety estimate.
...
_CURRENT_RUN_BUDGET_USD = DEFAULT_POLICY["run_budget_usd"]
...
class MeteredChatOpenAI(BrowserUseChatOpenAI):
    """ChatOpenAI with a per-run emergency budget and token metering."""
...
def _monthly_cost(db) -> float:
    try:
        ...
        # Fail closed on billing telemetry for scheduled jobs; caller handles the exception path.
        raise RuntimeError("Could not read the monthly LOCENIX LLM cost ledger from Supabase.")


def _persist_usage(db, job_id: str) -> None:
    try:
