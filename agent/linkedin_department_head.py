...
TABLES = {'people': 'tbloK0jz2X6ffr0D2','queue': 'tblnWGZEjrRD5suj4','run_reports': 'tblntmY8DXjSJWq8t'}
...
def airtable_records(table_id: str, max_records: int = 500) -> list[dict[str, Any]]:
    ...
    while len(records)<max_records:
        ...
        if not isinstance(payload,dict): break
        records.extend(payload.get('records') or []); offset=str(payload.get('offset') or '')
        if not offset: break
    return records[:max_records]

def supabase_rows(table: str, query: str) -> list[dict[str, Any]]:
    if not SUPABASE_URL or not SUPABASE_KEY: return []
    rows=get_json(f"{SUPABASE_URL}/rest/v1/{table}?{query}", {'apikey':SUPABASE_KEY,'Authorization':f'Bearer {SUPABASE_KEY}','User-Agent':'locenix-agent8'}, default=[])
    return rows if isinstance(rows,list) else []

def truthy(v): return v is True or str(v).strip().lower() in {'true','yes','1','paid','active'}
def iso_day(v):
...
def metrics_from_sources():
    ...
    today=datetime.now(timezone.utc).date(); cutoff=datetime.now(timezone.utc)-timedelta(hours=24)
    ...
    if runs:
        for role in role_health:
            ...
            if matching:
                rs=str(field(matching[0],'Run Status','')).lower()
                role_health[role]='HEALTHY' if rs in {'completed','success','successful'} else ('BLOCKED' if rs in {'failed','blocked'} else 'ATTENTION')
    return {'leads':leads,'qualified_leads':qualified,'active_conversations':active,'replies':replies,'positive_signals':positive,'open_followups':open_followups,'overdue_followups':overdue,'visibility_checks_offered':offered,'visibility_checks_accepted':accepted,'trials':trials,'paid_customers':paid,'queue_backlog':backlog,'failed_jobs':failed_jobs,'queued_jobs':queued_jobs,'account_risk':account_risk,'agent_health':{**role_health,'Browser Worker':'HEALTHY' if jobs else 'UNKNOWN'},'source_health':{'airtable':'AVAILABLE' if people or queue or runs else 'UNKNOWN','supabase':'AVAILABLE' if jobs else 'UNKNOWN'},'risk_job_ids':risk_hits[:5]}

def num(v): return v if isinstance(v,(int,float)) and not isinstance(v,bool) else None

def main():
    cfg=load(TASK)
    if not cfg.get('enabled'): print(json.dumps({'status':'disabled'})); return
    metrics=metrics_from_sources(); directive=get_json(DIRECTIVE_URL,default={}); priorities=[]
    if metrics['account_risk'] is True: priorities.append(('SYSTEM_SAFETY','Pause risky LinkedIn actions and escalate the observed account/security signal.','Browser Worker'))
    if (num(metrics['positive_signals']) or 0)>0: priorities.append(('POSITIVE_REPLIES','Prioritize positive replies and active qualified conversations before new prospecting.','Conversation / Inbox Agent'))
    if (num(metrics['active_conversations']) or 0)>0: priorities.append(('ACTIVE_CONVERSATIONS','Work active conversations before generating additional outreach volume.','Conversation Agent'))
    if (num(metrics['overdue_followups']) or 0)>0: priorities.append(('FOLLOWUPS','Clear overdue qualified follow-ups before new lead research.','Inbox / Follow-up Agent'))
    if (num(metrics['qualified_leads']) or 0)>0: priorities.append(('QUALIFIED_LEADS','Process existing qualified leads before increasing discovery volume.','Lead / Conversation Agent'))
    if not priorities: priorities.append(('GROWTH','No observed downstream bottleneck; keep conservative qualified lead generation active.','Lead Agent'))
