...
returns uuid language plpgsql security invoker set search_path=public as $$
declare j agent_jobs%rowtype;i company_incidents%rowtype;
begin
...
begin
 ...
 if provider_n>=3 then i:=company_incident_open(null,'ai_provider','provider_errors','AI_PROVIDER_ERROR_RATE','high','At least three AI provider failures in one hour','AI_PROVIDER_ERROR_RATE','deterministic');created:=created+1;end if;
 if routed>=5 and invalid_n::numeric/routed>=0.2 then i:=company_incident_open(null,'ai_validation','invalid_output_rate','AI_INVALID_OUTPUT_SPIKE','high','Invalid AI output rate exceeded 20 percent','AI_INVALID_OUTPUT_SPIKE','deterministic');created:=created+1;end if;
 if routed>=5 and strong_n::numeric/routed>=0.8 then i:=company_incident_open(null,'llm_router','strong_usage_rate','AI_STRONG_USAGE_SPIKE','high','Strong tier exceeded 80 percent of routed calls','AI_STRONG_USAGE_SPIKE','deterministic');created:=created+1;end if;
 ...
 if llm_today>1 then i:=company_incident_open(null,'cfo','cost_spike','AI_DAILY_COST_SPIKE','high','Daily conservative AI charge exceeded EUR 1','AI_DAILY_COST_SPIKE:'||p_now::date,'deterministic');created:=created+1;end if;
